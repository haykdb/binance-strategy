...
class FuturesTrader:
    ...
    def safe_futures_order(self, symbol: str, side: str, quantity: float, max_retries=10):
        attempt = 0
        ...
        while attempt < max_retries:
            try:
                ...
                if e.code in (-4131, -1013):
                    ...
                    logger.error(f"[FUTURES WARNING] ReduceOnly rejected (already closed). Safe to ignore.")
                    return True
                else:
                    logger.error(f"[ERROR] Order failed: {e.message}")
                    return False

        logger.error(f"[ERROR] Failed to place order after {max_retries} retries. Giving up.")
        return False

    def futures_open_long(self, symbol: str, quantity: float):
        self.futures_client.futures_create_order(symbol=symbol, side='BUY', type='MARKET', quantity=quantity)
        logger.info(f"[FUTURES] Opened LONG {quantity} {symbol}.")

    def futures_open_short(self, symbol: str, quantity: float):
        self.futures_client.futures_create_order(symbol=symbol, side='SELL', type='MARKET', quantity=quantity)
