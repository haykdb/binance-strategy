...
class SpotTrader:
    ...
    def margin_sell(self, symbol: str, quantity: float):
        if config.USE_TESTNET:
            ...
            self.safe_margin_order(symbol=symbol, side='SELL', type='MARKET', quantity=quantity)
    ...
    def margin_buy(self, symbol: str, quantity: float):
        if config.USE_TESTNET:
            ...
            self.safe_margin_order(symbol=symbol, side='BUY', type='MARKET', quantity=quantity)
    ...
    def safe_spot_order(self, symbol: str, side: str, quantity: float, max_retries=3, type="MARKET"):
        attempt = 0
        ...
        logger.error(f"[SPOT ERROR] Failed to place spot order after {max_retries} retries. Giving up.")
        return False

    from binance.exceptions import BinanceAPIException
    import time

    def safe_margin_order(self, symbol: str, side: str, quantity: float, max_retries=3, type="MARKET"):
        attempt = 0
        ...
        print(f"[MARGIN ERROR] Failed to place margin order after {max_retries} retries. Giving up.")
        return False
