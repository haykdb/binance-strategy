...
class OrderManager:
    ...
    def futures_buy(self, symbol: str, quantity: float, reduce_only: bool = False) -> bool:
        return self._safe_order(symbol, 'BUY', quantity, is_futures=True, reduce_only=reduce_only)

    def futures_sell(self, symbol: str, quantity: float, reduce_only: bool = False) -> bool:
        return self._safe_order(symbol, 'SELL', quantity, is_futures=True, reduce_only=reduce_only)

    def futures_order(self, symbol: str, side: str, quantity: float, reduce_only: bool = False) -> bool:
        assert side in ['BUY', 'SELL']
        return self._safe_order(symbol, side, quantity, is_futures=True, reduce_only=reduce_only)

    # === Public Position Closer ===
    def close_position(self, symbol: str, is_futures: bool) -> bool:
        return self.close_futures_position(symbol) if is_futures else self.close_spot_position(symbol)
    ...
    def close_futures_position(self, symbol: str) -> bool:
        try:
            ...
            return self._safe_order(symbol, side, quantity, is_futures=True, reduce_only=True)
    ...
    def close_spot_position(self, symbol: str) -> bool:
        try:
            ...
            spot_price = round(float(self.spot.get_symbol_ticker(symbol=symbol)["price"]), 4)
            quantity = 0.0
    ...
    def _safe_order(self, symbol: str, side: str, quantity: float,
                    is_futures: bool, reduce_only=False, max_retries=3) -> bool:
        attempt = 0
        ...
        while attempt < max_retries:
            ...
            try:
                ...
                logger.success(f"[ORDER SUCCESS] {side} {quantity} {symbol} (futures={is_futures}, reduceOnly={reduce_only})")
                return True

            except BinanceAPIException as e:
                if e.code in (-1013, -4131):
                    logger.warning(f"[ORDER WARNING] {e.message} (code {e.code}). Retrying smaller size...")
                    quantity /= 2
                    if quantity < min_qty:
                        print(f"[ORDER FAIL] Quantity {quantity} below minQty {min_qty}. Aborting.")
                        return False
                    attempt += 1
                    time.sleep(1)

                elif e.code == -2022:
                    logger.warning(f"[ORDER NOTICE] ReduceOnly rejected. Treating as success.")
                    return True

                else:
                    logger.error(f"[ORDER ERROR] Binance API error {e.code}: {e.message}")
                    return False
